...
def _worker_fixed(spectrum: SpectrumData, mode: str, model_key: str) -> BatchItemResult:
    quality = assess_spectrum_quality(spectrum, run_kk=False)
    hint = detect_segments(spectrum, mode=mode)
    fit = _fit_single_safe(spectrum, model_key, hint)
...
def _worker_auto(spectrum: SpectrumData) -> tuple[FitOutcome, FitOutcome, SegmentDetection, QualityReport]:
    quality = assess_spectrum_quality(spectrum, run_kk=False)
    ...
    single_fit = _fit_single_safe(spectrum, SINGLE_MODEL_KEY, single_hint)
    double_fit = _fit_single_safe(spectrum, DOUBLE_MODEL_KEY, double_hint)
...
def _analyze_batch_fixed_sequential(
    ...
    for index, spectrum in enumerate(spectra, start=1):
        item = _worker_fixed(spectrum, mode, model_key)
        items.append(item)
        if progress_callback is not None:
...
def _analyze_batch_auto_sequential(
    ...
    for index, spectrum in enumerate(spectra, start=1):
        single_fit, double_fit, auto_hint, quality = _worker_auto(spectrum)
        item, active_mode, pending_mode, pending_count = _choose_auto_item(
            spectrum,
            quality,
            single_fit,
...
def _fit_single_safe(
    ...
    try:
        return fit_spectrum(spectrum, model_key, segment_hint=hint, allow_fallback=True, auto_preprocess=False)
    except Exception as exc:
...
def _candidate_score(fit: FitOutcome, mode: str, auto_hint: SegmentDetection) -> float:
    ...
    score = _finite_or(fit.statistics.get("bic"), 1e9)
    if fit.status == "warn":
        score += 12.0

    chi2 = _finite_or(fit.statistics.get("chi2_reduced"), 10.0)
    score += min(chi2 * 4.0, 40.0)

    cond = fit.statistics.get("jtj_cond")
    if cond is not None:
        cond = float(cond)
